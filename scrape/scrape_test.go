...
func TestScrapeLoopDiscardDuplicateLabels(t *testing.T) {
	...
	testutil.Ok(t, err)
	series, err := q.Select(false, nil, labels.MustNewMatcher(labels.MatchRegexp, "__name__", ".*"))
	testutil.Ok(t, err)
	_, err = q.Exec()
	testutil.Ok(t, err)
	...
	q, err = s.Querier(ctx, time.Time{}.UnixNano(), 0)
	testutil.Ok(t, err)
	series, err = q.Select(false, nil, labels.MustNewMatcher(labels.MatchEqual, "le", "500"))
	testutil.Ok(t, err)
