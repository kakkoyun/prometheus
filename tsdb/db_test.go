...
func query(t testing.TB, q storage.Querier, matchers ...*labels.Matcher) map[string][]tsdbutil.Sample {
	ss, err := q.Select(false, nil, matchers...)
	defer func() {
...
func TestDB_Snapshot(t *testing.T) {
	...
	// sum values
	seriesSet, err := querier.Select(false, nil, labels.MustNewMatcher(labels.MatchEqual, "foo", "bar"))
	testutil.Ok(t, err)
...
func TestDB_e2e(t *testing.T) {
	...
	for _, qry := range queries {
		...
		for i := 0; i < numRanges; i++ {
			...
			ss, err := q.Select(false, nil, qry.ms...)
			testutil.Ok(t, err)
			ws, err := q.Exec()
			testutil.Ok(t, err)
			testutil.Equals(t, 0, len(ws))
...
func TestNotMatcherSelectsLabelsUnsetSeries(t *testing.T) {
	...
	for _, c := range cases {
		ss, err := q.Select(false, nil, c.selector...)
		testutil.Ok(t, err)
...
func TestDBReadOnly_FlushWAL(t *testing.T) {
	...
	// Sum the values.
	seriesSet, err := querier.Select(false, nil, labels.MustNewMatcher(labels.MatchEqual, defaultLabelName, "flush"))
	testutil.Ok(t, err)
...
func TestDBQueryDoesntSeeAppendsAfterCreation(t *testing.T) {
	...
	ss, err := querierBeforeAdd.Select(false, nil, labels.MustNewMatcher(labels.MatchEqual, "foo", "bar"))
	testutil.Ok(t, err)
	_, err = querierBeforeAdd.Exec()
	testutil.Ok(t, err)
	...
	ss, err = querierAfterAddButBeforeCommit.Select(false, nil, labels.MustNewMatcher(labels.MatchEqual, "foo", "bar"))
	testutil.Ok(t, err)
	_, err = querierAfterAddButBeforeCommit.Exec()
	testutil.Ok(t, err)
	_, seriesSet, err = expandSeriesSet(ss)
	testutil.Ok(t, err)
	testutil.Equals(t, map[string][]sample{}, seriesSet)
	...
	ss, err = querierAfterCommit.Select(false, nil, labels.MustNewMatcher(labels.MatchEqual, "foo", "bar"))
	testutil.Ok(t, err)
	_, err = querierAfterCommit.Exec()
	testutil.Ok(t, err)
	_, seriesSet, err = expandSeriesSet(ss)
	testutil.Ok(t, err)
